...
**⚠️ RESET acté le 2026-07-23 (session 8, plus tard le même jour) — décision explicite d'Arnaud, tout ce qui suit jusqu'à nouvel ordre est en cours de remplacement, pas encore réécrit en détail** :
- Le travail de production Sorceress documenté ci-dessous (sections 1 à 8, set herbe/chemin en 9 fichiers déjà intégré dans GDevelop) est **ignoré** — pas supprimé du jeu pour l'instant, mais plus la méthode de référence.
- Nouvel outil : **PixelLab** (abonnement à prendre au Tier 1, 12$/mois, pour débloquer `create_building_kit` — testé et confirmé bloqué en trial avec l'erreur "Tier 1 is required for this"). `create_topdown_tileset` (sol, grammaire Wang automatique) et `create_map_object` (props) fonctionnent déjà en trial.
- **Style validé par Arnaud (2026-07-23)** : vue du dessus 2D avec léger inclinement (comme TLR), **pas** un rendu strictement plat. Contredit l'ancienne persona section 7 ("strict 3/4 top-down... only the roof visible, never side walls") — cette règle est caduque. Paramètre PixelLab confirmé : `view: "high top-down"` (~35°), `outline: "lineless"`, `shading: "flat shading"` — testé sur un puits, un tonneau et un étal de marché, tous validés par Arnaud.
- Le reste du document (palette section 6, grammaire d'autotiling section 4) reste probablement en partie réutilisable (la palette hex ne dépend pas de l'outil, la grammaire Wang est de toute façon automatisée nativement par `create_topdown_tileset`) — à confirmer au fur et à mesure, pas encore validé explicitement.
- Réécriture complète prévue une fois la méthode PixelLab validée de bout en bout (sol + props + bâtiments), pas avant.
- **Kit de bâtiment modulaire (`create_building_kit`) abandonné (2026-07-23, même session)** : testé (56 pièces, 20 générations), mais assemblage manuel bloqué — les pièces (canevas 52×87px, murs 2 tuiles de haut) n'ont pas de convention d'ancrage documentée côté PixelLab (vérifié via `agent_help`, aucune info), et un aperçu composite construit à partir des coordonnées mesurées dans les pixels montre de vrais espaces entre pièces, pas juste un défaut d'accrochage. **Décision d'Arnaud : abandonner la modularité, générer chaque maison comme un objet unique pré-composé via `create_map_object`** (même méthode que le puits/tonneau/étal, déjà validée sans problème d'alignement). Objets Sprite/ressources créés pour le kit retirés du JSON (nettoyage complet). **Bâtiments explicitement reportés à après le sol** (priorité initiale du Tier 1 = les tuiles) — à reprendre plus tard, pas maintenant.
- Grille d'édition GDevelop (`uiSettings` de la scène `Niveau1_Village`) : `grid`/`snap` activés à 32×32 (2026-07-23) — utile pour tout placement futur, indépendant de la décision ci-dessus.
- **Maisons pré-composées validées (2026-07-23)** : 3 variantes générées via `create_map_object` mode **basique** (sans `background_image`), 64×64, `view: high top-down`, `outline: lineless`, `shading: flat shading` — `MaisonPierre`, `MaisonColombage`, `MaisonBeigeFleurs` (voir `structure_json_projet_x.md`). **Leçon retenue sur la palette** : un premier essai avec `background_image` (extrait du tileset herbe, mode style-matching) a produit des murs sombres non cohérents avec la palette village — le contexte "herbe seule" n'apporte aucune information sur les couleurs de pierre/bois/toit, le modèle a improvisé. Le mode basique avec une description explicite nommant les couleurs souhaitées (ex. "warm pale stone walls, golden-brown thatched roof... no dark or black colors") a donné un résultat cohérent, comme pour le puits/tonneau/étal. **Ne pas utiliser `background_image` avec un contexte qui ne contient pas les couleurs du matériau à générer.**

**Correction d'angle, tentative 1 — invalidée (2026-07-23, même session)** : hypothèse posée que `view: "low top-down"` (~20°) corrigeait le rendu isométrique des maisons. **Jamais appliquée aux 3 maisons réelles** (restées en `high top-down` dans la scène commitée) et, testée a posteriori le 2026-07-24, ne corrige pas le problème : rendu encore clairement isométrique (dessus de toit + mur latéral visibles). Cette piste est invalidée, ne pas la reprendre.

**Correction d'angle, tentative 2 — validée par Arnaud (2026-07-24)** : le vrai problème n'est pas l'angle top-down (20° vs 35°) mais le fait qu'un bâtiment avec hauteur rendu en top-down montre toujours au moins un pan de toit + un mur latéral → lecture "isométrique-boîte" quelle que soit la valeur de `view` dans la famille top-down. **Fix : `view: "side"`** (vue de face pure) — convention standard des RPG top-down (Zelda, Stardew Valley) où le sol est vu de dessus mais les bâtiments sont dessinés de face, en rupture de perspective assumée. Donne une façade seule (porte, fenêtre, pignon de toit vu de face), sans mur latéral ni dessus de toit. **`view: "side"` est désormais la valeur à utiliser pour tout bâtiment ayant une façade identifiable** (maison, plus tard boutique/donjon) — reste `high top-down` pour les props sans façade (puits, tonneau, étal), toujours validés tels quels. Les 3 maisons (`MaisonPierre`, `MaisonColombage`, `MaisonBeigeFleurs`) ont été régénérées dans ce nouveau style (`maison_*_v2.png`) et remplacent les anciennes dans le JSON.

**Leçon de process capitalisée le 2026-07-24** (voir mémoire Claude Code `feedback_pixellab_asset_saving`) : une génération jugée bonne pendant un test de comparaison n'avait jamais été écrite sur disque — perdue (les map objects PixelLab auto-suppriment côté serveur après 8h, aucun historique récupérable). Règle actée : toute génération retenue s'écrit immédiatement dans `game/assets/`, jamais laissée comme aperçu éphémère.

**Référence de qualité de rendu ajoutée (2026-07-24)** : `game/assets/Reference_Style_Village.png` (transmise par Arnaud, dense façon Stardew Valley). **Clarifié par Arnaud** : ce n'est pas une cible de densité pour le niveau 1 (qui reste "dégagé", décision du 2026-07-23 inchangée) — c'est une référence de **qualité technique de rendu** (richesse des couleurs, ombrage, cohérence entre assets). Arnaud note que cette densité **pourrait convenir au niveau 2**, qui a déjà les obstacles défensifs prévus (`CONCEPT_JEU.md`) — un village dense s'accorderait naturellement avec ce mécanisme. Pas une décision figée à ce stade, juste une piste à garder pour quand le niveau 2 sera abordé.

**⚠️ Génération de bâtiments (maisons) via PixelLab `create_map_object` — mise en pause (2026-07-24), décision d'Arnaud.** Après plusieurs tentatives (`view: side` validé pour Pierre et Colombage, mais MaisonBeigeFleurs reste systématiquement biaisée rose/pastel malgré 2 reformulations de prompt, y compris en retirant toute mention de couleur/fleurs), Arnaud tranche : on n'obtient pas le résultat voulu avec cette méthode, pas la peine de s'acharner. **Décision : arrêter la génération d'objets (bâtiments) pour l'instant, se concentrer sur les assets de tiles (sol, et ce qui suit : obstacles niveau 2, etc.), les maisons seront traitées plus tard via un autre outil/une autre méthode (non choisie à ce stade).** Les 2 maisons déjà validées (`MaisonPierre` v2, `MaisonColombage` v3) restent en l'état dans le jeu ; `MaisonBeigeFleurs` reste sur son dernier essai (v4) en attendant la reprise du sujet bâtiments.

### Sol — production réelle intégrée (2026-07-23, même session)

**Deuxième tileset généré** : herbe↔pavés (place), chaîné sur la **même tuile d'herbe** que herbe↔chemin via `upper_base_tile_id` (voir transcription PixelLab "Generating pixel art tilesets" — technique "reference tiles" pour garantir une herbe identique entre plusieurs tilesets). Confirmé par les métadonnées : mêmes positions de coin entre les deux tilesets, art d'herbe identique.

**Atlas combiné et intégré dans GDevelop** : `sol_atlas_pixellab.png` (128×256, 4×8, 32 tuiles = les deux tilesets 4×4 empilés) remplace `sol_atlas.png` (Sorceress, obsolète). Objet `NewTileMap` reconfiguré (`rowCount: 8`, `columnCount: 4`). Les 2 instances déjà placées (herbe pleine scène + chemin peint par Arnaud) ont eu leurs index de tuiles remappés vers le nouvel atlas plutôt que d'être effacées — le tracé du chemin déjà peint est conservé. Détail des index utiles dans `structure_json_projet_x.md`.

**Reste à faire par Arnaud** : recharger le projet, peindre la place pavée avec la nouvelle section de la palette (lignes 4-7 de l'atlas) là où voulu.

### Scène v1 complète — nettoyage + construction autonome (2026-07-23, même session)

**Demande d'Arnaud** : nettoyer tout ce qui est obsolète (objets Sorceress, kit de bâtiment abandonné) et proposer une scène v1 complète, en remplacement des `RepereDecor` (marqueurs gris temporaires). Fait intégralement — détail des objets/positions dans `structure_json_projet_x.md`.

**Méthode de peinture du sol** : script utilisant les métadonnées exactes des tilesets Wang (corners NW/NE/SW/SE par tuile, extraites de l'API PixelLab) pour calculer le bon index de tuile à chaque cellule d'un réseau de sol arbitraire (route + branche + place), plutôt que de deviner ou de peindre à la main tuile par tuile. Reproductible pour toute extension future du réseau.

**Statut** : scène testée et validée par Arnaud dans l'éditeur (s'ouvre, tout s'affiche). Prochaines générations à orienter par les retours d'Arnaud sur cette v1 (assets manquants à identifier au cas par cas, pas par anticipation).
