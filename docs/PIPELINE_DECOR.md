...
**Correction d'angle (2026-07-23, même session)** : les 3 premières maisons (`view: "high top-down"`, mode basique) rendaient visuellement "isométrique" (toit à facettes multiples, plus de profondeur) comparé à un premier essai avec `background_image` qui rendait plus plat/aligné à la grille. Reproduit et confirmé en comparant les deux images pixel à pixel. **Cause isolée : le paramètre `view`, pas le mode (basique vs style-matching).** `view: "low top-down"` (~20°) en mode **basique** (sans `background_image`) donne un rendu plat cohérent, sans les complications de fiabilité du mode style-matching (base64 volumineux = échecs de décodage répétés en pratique, éviter pour des images de fond au-delà d'environ 64×64). **Valeur à utiliser par défaut pour tout prop/bâtiment futur : `view: "low top-down"`** — remplace la valeur par défaut utilisée initialement (`"high top-down"`) pour le puits/tonneau/étal, qui n'a pas montré le même défaut visuel sur des props sans toit mais reste à surveiller si régénérés.
