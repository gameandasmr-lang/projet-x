...
- `Bâtiments` : **MaisonPierre, MaisonColombage, MaisonBeigeFleurs** — 3 variantes de maison pré-composées en une seule image chacune (PixelLab `create_map_object`, mode basique, 64×64, `view: side` — fichiers `maison_pierre_v2.png`/`maison_colombage_v2.png`/`maison_beige_fleurs_v2.png`, régénérés le 2026-07-24 pour corriger un rendu isométrique-boîte des premières versions en `high top-down`, voir `PIPELINE_DECOR.md`), remplace l'approche modulaire `create_building_kit` abandonnée. Origine bas-centre (32,64).
